...
// DHT Humidty & Temperature Sensor
#include <DHT22.h>
#define pinDATA 4
DHT22 dht22(pinDATA); 
// DHT Humidty & Temperature Sensor
...
/* --- Internal Functions --- */
static float get_sensor_temperature() 
{
  float t = dht22.getTemperature(true);
  t = t * 1.8 + 32;
...
static float get_sensor_humidity() 
{ 
    ...
    size_t* payload_buffer_length)
{
  ...
  temperature = get_sensor_temperature(); //temperature + x; //simulated_get_temperature();
  humidity = get_sensor_humidity();   //simulated_get_humidity();
